...
def inception_v4(num_classes, dropout_prob, weights, include_top,IMAGE_SIZE):
    '''Creates the Inception_v4 network.'''
    init = Input((IMAGE_SIZE[0], IMAGE_SIZE[1], IMAGE_SIZE[2])) # Channels last, as using Tensorflow backend with Tensorflow image dimension ordering
    ...
    # Dropout
    x = Dropout(dropout_prob)(x) # Keep dropout 0.2 as mentioned in the paper
    x = Flatten()(x) # Output: 1536

    # Output layer
    output = Dense(units = num_classes, activation = "softmax")(x) # Output: 1000
...
def create_model(num_classes, dropout_prob, weights, include_top,IMAGE_SIZE):
    return inception_v4(num_classes, dropout_prob, weights, include_top,IMAGE_SIZE)
